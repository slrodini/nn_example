...
\begin{center}
\begin{minipage}{10cm}
\setbeamercolor{postit}{fg=black,bg=myNewColorA}
...
\begin{figure}
    \centering
    \includestandalone[width=0.65\textwidth]{Notes/Figures/network}
\end{figure}
\end{frame}
...
\pause
1) define a function (loss function) that tell us how far away from data we are now: $\sim \sum (T_i - E_i)^2$\\
\pause
2) vary in turn all the parameters of the network: multidimensional `mountain-valley' picture \\
\pause
3) move in the direction in which the loss function decrease more rapidly
\end{frame}
